    ...
    constructor() {
      ...
      editableListContainer.innerHTML = `
        ...
        <ul class="item-list">
          ${listItems.map(item => `
            <li>${item}
              ${this.disableRemoval ? '' : '<button class="editable-list-remove-item icon">&ominus;</button>'}
            </li>
          `).join('')}
        </ul>
        <slot></slot>
...
//     }
    ...
    get disableRemoval(){
      return this.getAttribute('disableRemoval') || false;
    }
...
const changeAttrOnToDoList = () => {
  const el = document.getElementById('main')
  const btn = document.querySelector('button')
  btn.textContent = 'clicked22'
  el.setAttribute('title', 'Hello OTUS')
  el.setAttribute('title2', 'Hello OTUS')
  console.log('el',el)
